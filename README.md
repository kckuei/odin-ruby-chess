...
A `ChessGame` requires `Players`  that interact with a `ChessBoard`. A `Serializer` to save the state of the game, and `Logger` to record move history is also included. A `ChessBoard` contains various chess piece objects (e.g., the `Pond`, `Knight`, `Bishop`, etc.). The board is represented as a nested array, where blank strings represent empty spaces, or otherwise are occupied by chess piece objects. Each chess piece has instance variables (attributes) or knowledge of which `Player` it belongs to, it's position/location on the `ChessBoard`, its visual avatar (e.g. ♞) when printed, etc. Each chess piece also has its own methods/rules defined for what constitutues a valid move and how it's able to move across the playing field.
